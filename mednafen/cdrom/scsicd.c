...
static PCFX_RawAudioMixer *cdda_mixer;
...
static SCSI_FIFO *SCSI_FIFO_Create(uint32 size)
{
 SCSI_FIFO *f = (SCSI_FIFO*)calloc(1, sizeof(SCSI_FIFO));
 if(!f) return NULL;
 f->data = (uint8*)calloc(size, 1);
 if(!f->data) { free(f); return NULL; }
 f->size = size;
 return f;
}

static void SCSI_FIFO_Destroy(SCSI_FIFO *f)
{
 if(!f) return;
 free(f->data);
 free(f);
...
static void SCSI_FIFO_Flush(SCSI_FIFO *f)
{
 f->read_pos = f->write_pos = f->in_count = 0;
}

static uint32 SCSI_FIFO_CanWrite(const SCSI_FIFO *f)
{
 return f->size - f->in_count;
}

static void SCSI_FIFO_Write(SCSI_FIFO *f, const uint8 *src, uint32 len)
{
 while(len-- && f->in_count < f->size)
 {
...
static uint8 SCSI_FIFO_ReadByte(SCSI_FIFO *f)
{
 uint8 ret = 0;
 if(f->in_count)
 {
  ret = f->data[f->read_pos];
...
static void VirtualReset(void)
{
	...
	cdda.last_sample[0] = cdda.last_sample[1] = 0;
	cdda.CDDADiv = 0;
...
static void GenSubQFromSubPW(void)
{
	...
	for(uint_fast8_t i = 0; i < 96; i++)
	SubQBuf[i >> 3] |= ((cd.SubPWBuf[i] & 0x40) >> 6) << (7 - (i & 7));

	if(!subq_check_checksum_C(SubQBuf))
	{
	}
	else
	{
...
{
	SCSI_FIFO_Write(din, data_in, len);
...
#define SCSI_GET_4TH_ARG(_1,_2,_3,NAME,...) NAME
#define SCSI_GET_5TH_ARG(_1,_2,_3,_4,NAME,...) NAME
#define CommandCCError1(k) CommandCCError_Impl((k), 0, 0)
#define CommandCCError2(k,a) CommandCCError_Impl((k), (a), 0)
#define CommandCCError3(k,a,q) CommandCCError_Impl((k), (a), (q))
#define CommandCCError(...) SCSI_GET_4TH_ARG(__VA_ARGS__, CommandCCError3, CommandCCError2, CommandCCError1)(__VA_ARGS__)
#define DoPABase2(lba,len) DoPABase_Impl((lba), (len), CDDASTATUS_PLAYING, PLAYMODE_NORMAL)
#define DoPABase3(lba,len,status) DoPABase_Impl((lba), (len), (status), PLAYMODE_NORMAL)
#define DoPABase4(lba,len,status,mode) DoPABase_Impl((lba), (len), (status), (mode))
#define DoPABase(...) SCSI_GET_5TH_ARG(__VA_ARGS__, DoPABase4, DoPABase3, DoPABase2)(__VA_ARGS__)
   ...
   case 0x2B:
	    {
             ...
             float rate = 44100 + (float)44100 * speed / 100;
             //printf("Speed: %d %f\n", speed, rate);
...
static void DoREADHEADER10(const uint8 *cdb)
{
 ...
 CDIF_ReadRawSector_C(Cur_CDIF, raw_buf, HeaderLBA);	//, HeaderLBA + 1);
...
static void DoPABase_Impl(const uint32 lba, const uint32 length, unsigned int status, unsigned int mode)
{
 ...
 else
 {
  ...
  {
   CDIF_HintReadSector_C(Cur_CDIF, read_sec);	//, read_sec_end, read_sec_start);
  }
 }
...
void SCSICD_GetCDDAValues(int16 *left, int16 *right)
{
 if(cdda.CDDAStatus)
 {
  if(left) *left = cdda.CDDASectorBuffer[cdda.CDDAReadPos * 2];
  if(right) *right = cdda.CDDASectorBuffer[cdda.CDDAReadPos * 2 + 1];
 }
 else
 {
  if(left) *left = 0;
...
static inline int32 scale_cdda_delta(int32 v)
{
...
static inline void RunCDDA(uint32 system_timestamp, int32 run_time)
{
	...
	if(cdda.CDDAStatus == CDDASTATUS_PLAYING || cdda.CDDAStatus == CDDASTATUS_SCANNING)
	{
		...
		while(cdda.CDDADiv <= 0)
		{
			...
			if(cdda_mixer)
				PCFX_RawAudio_AddDelta(cdda_mixer, synthtime, scale_cdda_delta(sample[0] - cdda.last_sample[0]), scale_cdda_delta(sample[1] - cdda.last_sample[1]));
		...
		/* The Blip buffer path masked this by its DC blocker, but a direct
		   mixer must explicitly return CD-DA to zero when playback stops;
		   otherwise the last sector sample becomes a permanent DC level until
		   the next playback command. */
		if(cdda.CDDAStatus == CDDASTATUS_STOPPED && (cdda.last_sample[0] || cdda.last_sample[1]))
		{
			uint32 synthtime = system_timestamp / cdda.CDDATimeDiv;
			if(cdda_mixer)
				PCFX_RawAudio_AddDelta(cdda_mixer, synthtime, scale_cdda_delta(-cdda.last_sample[0]), scale_cdda_delta(-cdda.last_sample[1]));
			cdda.last_sample[0] = 0;
			cdda.last_sample[1] = 0;
		}
	}
}
	...
	if(CDReadTimer > 0)
	{
		...
		if(CDReadTimer <= 0)
		{
			...
			else
			{
				...
				else if(!CDIF_ReadRawSector_C(Cur_CDIF, tmp_read_buf, SectorAddr))	//, SectorAddr + SectorCount))
				{
					cd.data_transfer_done = false;
				...
				else if(ValidateRawDataSector(tmp_read_buf))
				{
					...
					if(tmp_read_buf[12 + 3] == 0x2)
						SCSI_FIFO_Write(din, tmp_read_buf + 24, 2048);
					else
						SCSI_FIFO_Write(din, tmp_read_buf + 16, 2048);
...
uint32 SCSICD_Run(scsicd_timestamp_t system_timestamp)
{
	...
	{
		if(SEL_signal) ChangePhase(PHASE_COMMAND);
	}
		...
		switch(CurrentPhase)
		{
		  ...
		  case PHASE_DATA_IN:
			if(!REQ_signal && !ACK_signal)
			{
			 ...
			 {
			  cd_bus.DB = SCSI_FIFO_ReadByte(din);
			  SetREQ(true);
			 }
			}
			if(REQ_signal && ACK_signal)
	...
	if(cdda.CDDAStatus == CDDASTATUS_PLAYING || cdda.CDDAStatus == CDDASTATUS_SCANNING)
	{
		int32 cdda_div_sexytime = (cdda.CDDADiv + (cdda.CDDADivAcc) + ((1 << 20) - 1)) >> 20;
...
void SCSICD_Close(void)
{
	if(din)
	{
		SCSI_FIFO_Destroy(din);
		din = NULL;
	}
}

void SCSICD_Init(int type, int cdda_time_div, PCFX_RawAudioMixer *audio_mixer, uint32 TransferRate, uint32 SystemClock, void (*IRQFunc)(int), void (*SSCFunc)(uint8, int))
{
	Cur_CDIF = NULL;
	TrayOpen = true;
...
int SCSICD_StateAction(StateMem* sm, const unsigned load, const bool data_only, const char *sname)
{
	SFORMAT StateRegs[] = 
	{
	  ...
	  SFARRAYN(din->data, din->size, "din_fifo"),
	...
	int ret = MDFNSS_StateAction(sm, load, data_only, StateRegs, sname, 0);
