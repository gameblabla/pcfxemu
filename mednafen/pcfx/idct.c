...
static inline void IDCT_1D(int32* MDFN_RESTRICT c_in, int32* MDFN_RESTRICT o, unsigned psh)
{
...
static inline void IDCT_1D_Multi(int32* MDFN_RESTRICT c, int32* MDFN_RESTRICT o, unsigned psh)
{
 for(unsigned i = 0; i < 8; i++)
  IDCT_1D(&c[i * 8], &o[i], psh);
}
...
void IDCT(int32* c)
{
 ...
 _Static_assert(IDCT_PRESHIFT == EFF_RSHIFT_2D, "IDCT_PRESHIFT != EFF_RSHIFT_2D");

 IDCT_1D_Multi(c, buf, 0);
 IDCT_1D_Multi(buf, c, EFF_RSHIFT_1D_POST);
}
