...
const void* pcfx_headless_video_pixels(int* width, int* height, int* pitch_pixels);
...
extern bool SaveState(char* path, uint_fast8_t state);
...
static bool resolve_homebrew_path(const char* input, char* out, size_t out_size)
{
    ...
    while((ent = readdir(dp)))
    {
        ...
        if(!has_huexe_extension(ent->d_name))
            continue;
        char candidate[1024];
        snprintf(candidate, sizeof(candidate), "%s/%s", input, ent->d_name);
        if(file_exists(candidate) && (!best[0] || strcmp(candidate, best) < 0))
...
static void make_game_name(const char* path)
{
    ...
    snprintf(GameName_emu, sizeof(GameName_emu), "%.*s", (int)sizeof(GameName_emu) - 1, base ? base : "pcfx");
}
...
void pcfx_headless_audio_write(const int16_t* samples, uint32_t frames)
{
...
int pcfx_headless_load_cd(PCFX_Headless* emu, const char* cd_path)
{
    ...
    if(!cd_path || !cd_path[0])
        return pcfx_headless_boot_bios(emu);
    char resolved_path[1024];
    if(!resolve_homebrew_path(cd_path, resolved_path, sizeof(resolved_path)))
...
{
    if(!emu || !emu->loaded)
        return set_error(emu, "no running CD machine to swap");
    if(emu != g_active)
        return set_error(emu, "this build supports one active emulator instance at a time");
    char resolved_path[1024];
...
{
    if(!emu || !emu->loaded)
        return set_error(emu, "no system loaded");
    PCFX_SoftReset();
    emu->frame_count = 0;
    return 1;
}
...
{
    (void)emu;
    if(pixel_format) *pixel_format = pcfx_headless_video_pixel_format();
    if(bytes_per_pixel) *bytes_per_pixel = pcfx_headless_video_bytes_per_pixel();
    return pcfx_headless_video_pixels(width, height, pitch_pixels);
}
...
int pcfx_headless_dump_memory(PCFX_Headless* emu, const char* region, const char* path)
{
    ...
    {
        if(!PCFX_StateSerializeMalloc(&owned, &size) || !owned || !size)
        {
            free(owned);
...
int pcfx_headless_save_state(PCFX_Headless* emu, const char* path)
{
    if(!emu || !path)
        return 0;
    char mutable_path[1024];
    snprintf(mutable_path, sizeof(mutable_path), "%s", path);
    if(!SaveState(mutable_path, 0))
        return set_error(emu, "failed to save state: %s", path);
    return 1;
}
...
int pcfx_headless_load_state(PCFX_Headless* emu, const char* path)
{
    ...
    char mutable_path[1024];
    snprintf(mutable_path, sizeof(mutable_path), "%s", path);
    if(!SaveState(mutable_path, 1))
        return set_error(emu, "failed to load state: %s", path);
    return 1;
}
